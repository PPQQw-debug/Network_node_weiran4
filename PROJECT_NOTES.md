...
默认不要为普通更新新开分支。用户说“git / GitHub / 上传 / 推送”时，就是把当前功能最全、最新的版本直接提交并推送到这个仓库的当前主线分支。只有用户明确要求额外分支、PR 或实验分支时，才创建新分支。推送后在最终回复里明确说明推到了哪个分支。当前历史主线分支名是 `codex-upload-20260601`。
